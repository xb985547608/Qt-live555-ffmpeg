...
Boolean parseRTSPRequestString(char const* reqStr,
  ...
  // Skip over the prefix of any "rtsp://" or "rtsp:/" URL that follows:   rtsp://10.10.4.252:8554/
  //去除rtsp:// or rtsp:/
  unsigned j = i+1;
  //去除空白字符  j指向命令字后的第一个非空白字符
  ...
  for (; (int)j < (int)(reqStrSize-8); ++j) {
	...
	//去除rtsp://后的地址和通道，j指向地址和通道后的第一个字符'/'
	++j;
  ...
  //查找URL的后缀   在RTSP/1.0之前  i指向后缀的第一个字符
  parseSucceeded = False;
  for (unsigned k = i+1; (int)k < (int)(reqStrSize-5); ++k) {
  	//将K指向   RTSP/1.0  的第一个字符
