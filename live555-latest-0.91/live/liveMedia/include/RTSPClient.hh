...
protected:
  int fVerbosityLevel;//0:禁用输出 
  unsigned fCSeq; // sequence number, used in consecutive requests    连续请求中使用的序列号
  Authenticator fCurrentAuthenticator;
  Boolean fAllowBasicAuthentication;//允许基本身份验证标志
  netAddressBits fServerAddress;//服务器地址

private:
  portNumBits fTunnelOverHTTPPortNum;
  char* fUserAgentHeaderStr;//用户代理字符串
  unsigned fUserAgentHeaderStrLen;
  int fInputSocketNum, fOutputSocketNum;
  char* fBaseURL;//地址
  unsigned char fTCPStreamIdCount; // used for (optional) RTP/TCP
  char* fLastSessionId;//上一个媒体会话的ID
  unsigned fSessionTimeoutParameter; // optionally set in response "Session:" headers  会话超时参数
  char* fResponseBuffer;//响应缓冲区大小
  unsigned fResponseBytesAlreadySeen, fResponseBufferBytesLeft;//缓冲区使用大小、剩余大小
